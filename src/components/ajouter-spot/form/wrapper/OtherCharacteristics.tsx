...
const OtherCharacteristics = ({
    ...
    const { control, getValues, setValue, register, watch } =
        useFormContext<AddPlaceSchemaType>();
    console.log('watch :', watch());
    const { stepTwo } = useAddCoworkingStore();
        ...
        <div className='space-y-4'>
            <div className='mb-24 flex justify-between'>
                <Controller
                    ...
                    control={control}
                    defaultValue={stepTwo.establishmentType || ''}
                    render={({ field }) => (
                ...
                <Controller
                    control={control}
                    defaultValue={stepTwo.espressoPrice || ''}
                    name='espressoPrice'
                    render={({ field }) => (
                        <input
                            {...field}
            ...
            <div className='flex flex-wrap gap-2'>
                {equipmentOptions.map((option) => (
                    <Controller
                        key={option.value}
                        control={control}
                        defaultValue={stepTwo.facilities || []}
                        name='facilities'
                        render={({ field }) => (
                            <button
                                type='button'
